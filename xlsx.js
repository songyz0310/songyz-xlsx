...
(function make_xlsx(XLSX) {
	...
	var make_ssf = function make_ssf(SSF) {
		...
		function eval_fmt(fmt, v, opts, flen) {
			...
			if (dt != null) {
				let dateTime = dt.y + "-" + dt.m + "-" + dt.d + " " + dt.H + ":" + dt.M + ":" + dt.S;
				if (opts._dateType == 1) {
					return dateTime;
				} else if (opts._dateType == 2) {
	...
	function safe_format(p, fmtid, fillid, opts) {
		if (opts._dateType) {
			_ssfopts._dateType = opts._dateType
			_ssfopts._numberType = opts._numberType
		}
		try {
